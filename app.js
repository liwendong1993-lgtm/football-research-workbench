...
function openCombo(id){
  ...
  saveCombo.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();saveCombo.disabled=true;setComboStatus('正在保存…');try{const raw=[];$$('#comboContent .combo-match-select').forEach(box=>{const m=matches.find(match=>match.id===box.dataset.match),checked=[...box.querySelectorAll('.combo-option:checked')];if(!m||!checked.length)return;const opts=availableOptions(m,draftFor(m.id)),options=checked.map(ch=>{const [market,pick]=splitOptionValue(ch.dataset.value);return opts.find(o=>o.market===market&&o.pick===pick)}).filter(Boolean);if(options.length)raw.push({matchId:m.id,num:m.num,home:m.home,away:m.away,options})});const items=enforceSingleMarketPerMatch(raw);if(!items.length){setComboStatus('请至少选择一场比赛的一个选项','error');return}const all=state.combos[state.activeDate]||[],combo={id:existing?.id||uid(),name:$('#comboName').value.trim()||'未命名方案',items,time:new Date().toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'})};state.combos[state.activeDate]=existing?all.map(c=>c.id===existing.id?combo:c):[...all,combo];saveState();setComboStatus('组合方案已保存','success');renderCombos();try{dlg.close()}catch(closeError){console.warn('组合弹窗关闭失败',closeError);dlg.removeAttribute('open')}toast('组合方案已保存')}catch(error){console.error('保存组合方案失败',error);setComboStatus(`保存失败：${error?.message||'浏览器存储异常，请截图反馈'}`,'error')}finally{saveCombo.disabled=false}});
}
...
if('serviceWorker' in navigator&&location.protocol.startsWith('http')) navigator.serviceWorker.register('./sw.js?v=20260717-1718',{updateViaCache:'none'}).then(registration=>registration.update()).catch(console.error);
