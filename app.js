...
function openCombo(id){
  ...
  $('#saveCombo').onclick=()=>{const raw=[];matches.forEach(m=>{const checked=[...document.querySelectorAll(`.combo-option[data-id="${m.id}"]:checked`)];if(!checked.length)return;const opts=availableOptions(m,draftFor(m.id)),options=checked.map(ch=>{const [market,pick]=splitOptionValue(ch.dataset.value);return opts.find(o=>o.market===market&&o.pick===pick)}).filter(Boolean);raw.push({matchId:m.id,num:m.num,home:m.home,away:m.away,options})});const items=enforceSingleMarketPerMatch(raw);if(items.length<2){toast('组合方案至少选择两场');return}const all=state.combos[state.activeDate]||[],combo={id:existing?.id||uid(),name:$('#comboName').value.trim()||'未命名方案',items,time:new Date().toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'})};state.combos[state.activeDate]=existing?all.map(c=>c.id===existing.id?combo:c):[...all,combo];saveState();dlg.close();renderCombos();toast('组合方案已保存')};
}
...
function drawSingleComboPoster(combo){
  ...
  const prize=schemePrizeRange(items,2),metrics=comboMetrics(items),rows=items.map(item=>{const match=state.matches.find(m=>m.id===item.matchId)||{},groups=comboPosterGroups(item,match);return {item,match,groups,height:120+groups.reduce((n,g)=>n+g.lines*72,0)}});
...
function showSingleComboPoster(combo){posterMode='combo';if(!drawSingleComboPoster(combo))return;$('#posterDialog .modal-head h3').textContent='单条方案图预览';$('#posterDialog').showModal()}
...
function downloadPoster(){const a=document.createElement('a'),prefix=posterMode==='scan'?'全部扫盘':posterMode==='combo'?'单条方案':'足球研究';a.download=`${prefix}-${state.activeDate}.png`;a.href=$('#posterCanvas').toDataURL('image/png');a.click();toast('图片已生成')}
